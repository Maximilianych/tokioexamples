...
/// Время заполнения my_vec1 и my_vec2 примерно одинаковое и самое большое из всех
/// Для заполнения векторов нулями они используют метод insert() в цикле, который каждый раз проверяет, что индекс, в который происходит вставка, <= len массива,
/// что len < capacity, то есть что ещё есть память для заполнения, и отдельно, что индекс < len, на случай, если вставка происходит не в конец массива и нужно сдвинать все элементы справа
/// Все эти проверки оказывают влияние на время заполнения векторов
/// my_vec1 обычно заполняется чуть медленне 
/// Он создаётся с помощью new() с нулевым capacity, из-за чего если при вставке нового значения len == capacity,
/// то происходит реаллокация вектора для выделением большего объёма памяти для вектора, что также влияет на время
/// my_vec2 создаётся методом with_capacity(), с помощью которого сразу выделяется необходимый объём памяти, что позволяет избежать реаллокаций
/// При создании my_vec3 используется макрос vec!, что примерно в два раза быстрее использования цикла
/// Некорый поиск в интернете сказал, что этого выделяется память и используется быстрая операция (аналогичная memset) для установки значения 6 во все элементы
/// В четвертом блоке происходит не заполнения вектора, а итерация по значениям вектора, их копирование в elem и присваивание этой временной переменной 7
/// При запуске релизной сборке этот блок выбрасывается и замеры показывают почти нулевое время
/// my_vec4 создаётся заполненной нулями практически моментально, хотя способ создания аналогичен my_vec3
/// Тот же поиск в интернете скащал, что это должно быть связано с тем, что дорогостоящая операция обнуления для больших объёмов не происходит сразу,
/// программе выделяются вирутальные страницы, ведущие к странице, фактически заполненной нулями,
/// а новая физическая память выделяется и обнуляется ядром при первой попытке записи в соответствующую виртуальную страницу
fn example3() {
